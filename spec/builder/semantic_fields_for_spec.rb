  ...
  context "when FormBuilder.semantic_errors_link_to_inputs is true" do
    before do
      ...
      concat(semantic_form_for(@fred) do |builder|
        concat(builder.semantic_fields_for(:posts) do |nested_builder|
          concat(nested_builder.input(:title))
          concat(nested_builder.label(:title, id: 'label_for_title'))
        end)
      end)
    end
    ...
    it "when there are errors, it prefers the developer-set aria values" do
      @output_buffer = ActionView::OutputBuffer.new ''

      concat(semantic_form_for(@fred) do |builder|
        concat(builder.semantic_fields_for(:posts) do |nested_builder|
          concat(nested_builder.input(:title, :input_html => { 'aria-describedby' => 'label_for_title', 'aria-invalid' => 'false' }))
          concat(nested_builder.label(:title, id: 'label_for_title'))
        end)
      end)

      expect(output_buffer.to_str).to \
        have_tag 'input#author_posts_attributes_0_title[aria-describedby="label_for_title"]', \
        count: 1
      expect(output_buffer.to_str).to \
        have_tag 'input#author_posts_attributes_0_title[aria-invalid="false"]', \
        count: 1
    end
  end
end
