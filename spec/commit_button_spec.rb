...
describe 'SemanticFormBuilder#commit_button' do
  ...
  describe 'when the object responds to :new_record? (Rails 2, deprecated in 3)' do
    
    before do
      @new_post.stub(:respond_to?).with(:to_model).and_return("X")
      @new_post.stub(:respond_to?).with(:new?).and_return(false)
    ...
    it 'should call :new_record?' do
      ...
      @new_post.should_not_receive(:new?)
  ...
  describe 'when the object responds to :new? (Rails 3)' do
    
    before do
      @new_post.stub(:respond_to?).with(:to_model).and_return("X")
      @new_post.stub(:respond_to?).with(:new?).and_return(true)
      @new_post.stub(:respond_to?).with(:new_record?).and_return(false)
      @new_post.stub(:respond_to?).with(:persisted?).and_return(true)
    end
    
    it 'should call :new?' do
      @new_post.should_receive(:new?)
