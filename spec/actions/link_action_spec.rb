...
describe 'LinkAction', 'when cancelling' do
  ...
  context 'without a :url and not valid post' do
    before do
      @new_post.stub!(:errors).and_return(mock('errors', :any? => true))
      concat(semantic_form_for(@new_post) do |builder|
        concat(builder.action(:cancel, :as => :link))
      end)
    end
    ...
    it 'should render a submit type of input' do
      output_buffer.should have_tag('li.action.link_action a[@href="javascript:history.go(-2)"]')
    end
