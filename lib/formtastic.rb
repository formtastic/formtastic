  ...
  class SemanticFormBuilder < ActionView::Helpers::FormBuilder
    ...
    protected
      ...
      def check_boxes_input(method, options)
        ...
        input_name      = generate_association_input_name(method)
        no_hidden_input = options.delete(:no_hidden_input)
        value_as_class  = options.delete(:value_as_class)
        ...
        list_item_content = collection.map do |c|
          ...
          li_content = template.content_tag(:label,
            Formtastic::Util.html_safe("#{self.check_box(input_name, html_options, value, unchecked_value, no_hidden_input)} #{escape_html_entities(label)}"),
            :for => input_id
          )
      ...
      def check_box(input_name, html_options = {}, checked_value = "1", unchecked_value = "0", no_hidden_input = false)
        return template.check_box_tag(input_name, checked_value, html_options[:checked], html_options) if no_hidden_input
        super(input_name, html_options, checked_value, unchecked_value)
      end
